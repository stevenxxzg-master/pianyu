...
The fenced YAML below is the machine-readable source used by
`bin/collect_release_performance_baseline` so the automated gate compares
directly against the thresholds documented above.
...
flows:
  homepage_load:
    ...
    metrics:
      homepage_request_ms:
        ...
        threshold: 800
        missing: fail
      homepage_app_shell_ms:
        label: First visible app shell
        unit: ms
        operator: <=
        threshold: 2500
        missing: fail
  ...
  post_publish:
    ...
    metrics:
      status_create_ms:
        label: Status create request
        unit: ms
        operator: <=
        threshold: 1000
        missing: fail
      status_visible_ms:
        label: Status visible on home timeline
        unit: ms
        operator: <=
        threshold: 3000
        missing: fail
      async_drain_ms:
        label: Follow-up async work drain
        unit: ms
        operator: <=
        threshold: 30000
        missing: fail
      publish_web_cpu_percent:
        label: Web CPU during publish
        unit: percent
        operator: <
        threshold: 70
        missing: risk
      redis_evictions_delta:
        label: Redis evictions delta
        unit: count
        operator: ==
        threshold: 0
        missing: risk
  notifications:
    ...
    evidence:
      ...
      - spec/workers/unfilter_notifications_worker_spec.rb
    metrics:
      notifications_api_ms:
        label: Notifications API request
        unit: ms
        operator: <=
        threshold: 1000
        missing: fail
      notification_delivery_ms:
        ...
        threshold: 5000
        missing: fail
      notification_drain_ms:
        label: Notification merge/unfilter drain
        unit: ms
        operator: <=
        threshold: 60000
        missing: fail
      ...
      pg_pool_waiting_queries:
        label: Streaming PG waiting queries
        unit: count
        operator: ==
        threshold: 0
        missing: risk
      streaming_clients_steady_state:
        label: Connected clients/channels return to steady state
        unit: boolean
        operator: ==
        threshold: true
        missing: risk
  timeline_scrolling:
    label: Timeline scrolling
    evidence:
      - spec/requests/api/v1/timelines/home_spec.rb
      - app/javascript/mastodon/components/scrollable_list/index.jsx
    metrics:
      timeline_pagination_ms:
        label: Timeline pagination max observed
        unit: ms
        operator: <=
        threshold: 1000
        missing: fail
      timeline_append_render_ms:
        label: Appended items render
        unit: ms
        operator: <=
        threshold: 2000
        missing: risk
      timeline_http_ok:
        label: Timeline pagination avoids 429/5xx
        unit: boolean
        operator: ==
        threshold: true
        missing: fail
      timeline_pg_waiting_queries:
        label: Postgres waiting clients
        unit: count
        operator: ==
        threshold: 0
        missing: risk
      timeline_web_rss_bytes:
        label: Web RSS during timeline pagination
        unit: bytes
        operator: <
        threshold: 1288490188
        missing: risk
  background_jobs:
    ...
    metrics:
      background_drain_ms:
        ...
        threshold: 30000
        missing: fail
      scheduled_job_lag_ms:
        label: Scheduled job lag
        unit: ms
        operator: <=
        threshold: 60000
        missing: risk
      retry_dead_growth:
        label: Retry/dead counts growth
        unit: count
        operator: ==
        threshold: 0
        missing: risk
      sidekiq_rss_bytes:
        ...
        missing: risk
      redis_memory_limit_ratio:
        label: Redis memory as fraction of configured limit
        unit: ratio
        operator: <
        threshold: 0.7
        missing: risk
      background_pg_waiting_queries:
        label: Postgres waiting clients
        unit: count
        operator: ==
        threshold: 0
