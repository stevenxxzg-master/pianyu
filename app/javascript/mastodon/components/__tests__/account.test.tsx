...
describe('<Account />', () => {
  it('renders redesigned account card details and actions', async () => {
    const { container } = render(
      <Account id='1' withBio defaultAction='mute' />,
      {
        state: {
          accounts: {
    ...
    expect(
      container.querySelector('a.account__display-name')?.getAttribute('href'),
    ).toBe('/@testuser');
    ...
    expect(
      await screen.findByRole('button', { name: 'Mute notifications' }),
    ).toBeTruthy();
  });
